...
export default class extends React.Component {
	constructor() {
		...
		},selected: 'temperatureBg', area: 1, channel: {c: 1, w: 2}}
	}
	componentWillMount() {
	...
	render() {
		const {btnBg, selected, area, brightness, temperature, channel} = this.state;
		return (
			<div className={'color-circle'}>
				...
				<div style={{display: selected === 'temperatureBg' ? 'block' : 'none'}}>
					<Temperature onFetch={() => temperatureChange(area, {c: channel.c, w: channel.w}, temperature, brightness)} onChange={v => this.setState({temperature: v})} onSwitch={v => sceneChange(area, v ? 1 : 4)}/>
				</div>
				<div style={{display: selected === 'brightnessBg' ? 'block' : 'none'}}>
					<Brightness onFetch={() => temperatureChange(area, {c: channel.c, w: channel.w}, temperature, brightness)} onChange={v => this.setState({brightness: v})} onSwitch={v => sceneChange(area, v ? 1 : 4)}/>
				</div>
			</div>
		);
	}
